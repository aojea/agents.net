...
Firecracker's own `CONNECT <port>` handshake. Section 8.2 describes the
executed test. A VMM that exposes host `AF_VSOCK` instead requires the CID
mapping and per-VM listener discipline of Sections 2.2 and 4.3.
...
### 8.2 Firecracker Two-Sandbox Test
